...
public class FABToolbarLayout extends RelativeLayout {
	...
	private void parseAttrs(AttributeSet attrs) {
		...
		isAuto = a.getBoolean(R.styleable.FABToolbarLayout_fabToolbarAuto, true);
		...
		toolbarLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				if (toolbarLayout.getWidth() != 0 || toolbarLayout.getHeight() != 0) {
					...
					fabParams.bottomMargin = marginToSet;
					...
					layoutParams.height = toolbarSize.y + marginToSet;
	...
	public void show() {//TODO better handling of fast clicks
		if (!isFab) {
			return;
		}
		isFab = false;
